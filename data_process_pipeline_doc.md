...
v1.0.2 已将 RAW 工作从导入阶段延后到 Develop 阶段：导入时只提取嵌入式或轻量预览并写入本地状态，不执行完整解码、反相或自动片基估计；用户打开画面后，后台按需生成有界的半尺寸/目标长边 16 位线性代理，WebGL 预览、Auto Invert 和密度分析复用同一代理。代理缓存采用有界 LRU，避免连续浏览 RAW 时无界增长。代理尚未就绪被视为暂态状态，前端会短暂重试；真正的 LibRaw 解码错误仍会明确提示。

该实现保持相机 RAW 的采集域策略：LibRaw 输出使用固定的 16 位、线性伽马参数，显示反相和调色在 WebGL/CPU 管线中完成。它不是 DCP/ICC 相机特性化流程，也不意味着所有 RAW 压缩格式都受支持；具体兼容性仍取决于内置 LibRaw 版本和文件格式。
