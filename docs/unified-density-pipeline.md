...
同类遗留（本轮未改）：胶片范围面板里的「批量应用 → 片基去色罩与反相」仍然是逐字复制源帧的 `base_color`，并且不会把源帧的密度范围写进目标帧的 `params`（未标定卷的渲染端点取自 `params`，不取自 `pipeline_state.render_mapping`），因此该入口需要用同一套「在目标帧上重新测量」的机制单独修一遍。
