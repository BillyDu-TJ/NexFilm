...
- 取画面块的块均值 95 分位作为该帧的画面高光；整卷逐帧测量后取**最亮的一帧**，钳制在跨度的 `0.45–0.85`，写入 `DensityAnchors.highlight_fraction`，整卷共用同一个白点（保持“整卷一套映射”的既有承诺，不做逐帧自动色阶）。白点是整卷共用的显示端点，只要低于整卷最亮的一帧就会截掉那一帧的高光，因此它必须来自最亮帧；早期版本只在整卷均匀抽 5 帧取中位数，抽不到最亮帧时用户只能手动抬高 D-Max。测量在“整卷批处理”渲染任何一帧之前执行一次，逐帧结果缓存在帧上，同一会话内再次批处理不再重复解码；只有部分帧测量成功时不会把已有的白点往下改，以免漏测的亮帧被截断。单帧自动反相在没有整卷白点时仍按该帧的测量值建立白点，作为绕过整卷测量的兜底。
