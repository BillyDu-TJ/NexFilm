...
自动反相用 Roll 锚点决定显示端点的基准；Develop 中的 Master D-Min/D-Max 滑块是叠加在该基准上的单帧手动微调（保存为 `d_min_offset` / `d_max_offset`），预览与导出都按“锚点端点 + 微调”计算，因此锚点本身不会被改写，整卷映射在用户不动滑块时仍然一致。微调按各通道在跨度中的占比缩放后叠加：Roll 锚点为每个通道量出各自的跨度，若把同一个密度偏移直接加到三个通道上，各通道的增益会按不同比例改变，画面随之偏色（抬高 D-Max 会明显偏绿），因此在归一化显示域里微调始终只是一次整幅曝光／反差移动。
...
- 取该帧画面块里**最亮的那些块**（块均值分布的 99.9 分位，块本身已是 32×32 平均，所以单像素噪点与灰尘无法决定结果）作为该帧的画面高光；整卷逐帧测量后取**最亮的一帧**，钳制在跨度的 `0.45–1.0`，写入 `DensityAnchors.highlight_fraction`，整卷共用同一个白点（保持“整卷一套映射”的既有承诺，不做逐帧自动色阶）。白点是整卷共用的显示端点，只要低于整卷最亮的一帧就会截掉那一帧的高光，因此它必须来自最亮帧。上界取片头本身（跨度的 1.0）：实测中确有整卷最亮块落在跨度的 1.02（片头采样偏低），此时把上界压在 0.85 只会把已经量到的高光再截掉一次。测量在“整卷批处理”渲染任何一帧之前执行一次，逐帧结果缓存在帧上，同一会话内再次批处理不再重复解码；只有部分帧测量成功时不会把已有的白点往下改，以免漏测的亮帧被截断。单帧自动反相在没有整卷白点时仍按该帧的测量值建立白点，作为绕过整卷测量的兜底。

  历史：早期版本先取“块均值 95 分位”，再在整卷上抽 5 帧取中位数。两者都低估了画面高光（实测有云/亮墙的画面里有 6%–14% 像素落在该端点之上，云层直接平掉），用户只能逐帧抬高 D-Max 才能压住，而这正是下面 `D-Max/D-Min` 微调偏色的由来。`highlight_fraction` 是 Roll 的属性：解析器（`filter_anchors`）在按合同筛选锚点时必须一并保留它，否则导出等重建状态的路径会退回逐帧白点，导出的文件比预览更亮——这正是「导出不是最后编辑的样子」的原因。

### 6.2 几何：只有一条管线负责摆放画面

预览（WebGL）、导出（CPU `render_f32_shader_equivalent`）和缩略图（`generate_processed_thumbnail`）必须由同一份几何摆放画面。事实上有两个漏洞：CPU 渲染核心只做裁切与透视采样，不套 `rotate_90_count/flip_h/flip_v`（那是 `map_oriented_uv_to_source` 的职责，预览的几何矩阵正是用它生成的），而缩略图渲染器连裁切之外的几何都没有套，还按未旋转的坐标做裁切。结果就是：用户在 Develop 里翻转画面后，导出和缩略图各自呈现出“反的”或“不一致”的方向（实测某卷 `_DSC772*` 全部 `flip_v=true`，导出与预览正好上下颠倒）。现在导出渲染器在采样前走 `map_oriented_uv_to_source`（四分之一转还会交换输出宽高），缩略图在裁切前做同样的旋转与翻转，三处方向一致。`thumbnail_geometry_matches_the_render_mapping` 与 `export_render_places_the_frame_with_the_frame_geometry` 两个测试把这条约束固定下来。
...
### 19.6 指标本身的局限（避免下一步走错方向）

把同样的显示编码（`density.gamma = 2.2`）套到哈苏 `任务 _1233.fff` 上：帧均值由 (0.27, 0.27, 0.29) 升到 (0.51, 0.51, 0.51)，R/G 1.002、B/G 0.998，**指标比原来更中性**（原先 B/G 1.061）。但逐像素看图会发现天空从"明确的蓝天"变成近乎苍白的浅蓝，画面发灰——也就是丢掉了用户认可的观感。

原因是"帧均值通道比"这个指标同时包含了**场景本身的颜色**与**偏色**：哈苏那张本来就是粉裙 + 蓝天 + 绿草，均值中性并不等于观感正确；lr 合并那张（混凝土、灰路面）本身接近中性，均值才适合当偏色指标。

因此后续验收必须按画面类型分别处理：对 lr 合并这类以中性物为主的场景可用帧均值与参考图对比；对哈苏这类高饱和场景必须用区域采样（天空、主体、草地）加人眼判断，不能只看全局均值。方案 D 的改造也不应以"把全局均值压到 1.0"为目标，而应以"中性物体的高光与中间调保持中性、场景本来的彩色不被洗掉"为目标。

### 19.7 本轮进一步排除的路径

- **极端处降饱和（chroma roll-off）**：在密度归一化后按"离中灰的距离"收缩色度（强度 0.25 / 0.50）。天空的红由 0.235 升到 0.333 / 0.413，暗部品红减轻，但整体 B/G 几乎不动（1.324 → 1.328 / 1.330）。说明**蓝色偏多不在极端处**，而是来自天空（蓝通道被裁）与中间调本身。
- **分析区泄漏**：最致密的红（T=0.107）位于分析区内 uv(0.185, 0.394)，不是片边或字符；全帧 142.8 万采样中有 95.1 万在分析区内。因此红通道的窗口由真实场景内容决定，不是统计缺陷。

当前残余（R/G 1.030、B/G 1.324）已经无法用窗口、偏移、参考点、显示编码、色域压缩或极端降饱和中的任何单项在不动其他画面的前提下消除：它需要一套逐通道响应模型（Phase 6 的胶片档案 / H-D 曲线），或对显示编码这类会改变所有画面观感的默认值做出取舍。这两项都超出本轮授权范围（用户已明确要求 D 日后再做），必须先由用户确认方向。

### 19.8 Loose Import 相对 v1.0.2 的偏色回归（根因，必须修）

现象：散片导入的校色与 v1.0.2 有明显差异，并且**随画面内容变化**——偏暖的场景（寺庙、黄墙）偏青，偏冷/偏绿的场景（河岸、树林）偏品红。同一卷不同画幅的偏移方向甚至相反。

决定性实验（对同一张底片人为施加整幅暖色 ×1.18 R / ×0.85 B，再分别渲染）：

| 路径 | 未加色 | 加暖色 |
| --- | --- | --- |
| v1.0.2 | R/G 1.028、B/G 0.774 | R/G 1.002、B/G 0.774（有响应） |
| 当前 Smart Auto | R/G 1.092、B/G 0.965 | R/G 1.092、B/G 0.965（**逐位不变**） |

机制：`share_smart_auto_density_scale` 的逐通道对齐取自**内容窗口的中心**，本质是灰世界；而 `pipeline_base_density` 在 Smart Auto 没有物理锚点时直接返回 `[0.0; 3]`（commands.rs:2349），也就是**实测片基根本没有参与渲染**。于是：

- 任何整幅偏色（扫描仪白平衡、光源色、过期卷偏色）都会被完全抵消；
- 任何以某色为主的场景会被补偿成反向偏色；
- v1.0.2 用的是逐通道片基扣除，与画面内容无关，所以稳定。

这同时解释了此前两个"改动没效果"的观测：开关机内白平衡、切换显示编码，对哈苏渲染几乎没有影响——差异同样被这套内容对齐抵消掉了。

修复方向：

1. 让实测片基成为散片的**逐通道密度参考**（密度减片基 + 三通道共享窗口 + 零偏移），把内容中心对齐降级为无片基时的兜底；
2. 片基估计本身需要更稳健：优先使用可见片边/橙色色罩，其次才是画面内最亮分位。实测直接用当前的最亮分位片基会**过扣**（`5.3-1.tif` 上 B/G 只剩 0.457，而 v1.0.2 是 0.774），所以第一步必须先修估计；
...
### 19.10 已实施：逐通道响应补丁下线，窗口回到逐通道同像素端点（窗口规则 3）

经过三轮实测（逐通道跨度响应 → 输入域端点证据门 → 用户观感裁决），结论是：**问题不在补偿参数，而在「三通道共享一个密度跨度」这一步**。

- 规则 2 的逐通道跨度响应解决了 `lr合并\*Pano.tif` 的青/品红崩坏，但判据无法区分「上游削平通道」与「画面本身以某色为主」，对正常素材写入了 90% 以上像素。
- 规则 3 第一版把它改成输入域端点证据门（端点堆积 ≥15% 且 ≥5× 其他通道），正常素材确实不再被改，但**全部退回共享跨度**，用户实测富士 RAF 出现紫蓝、尼康 NEF 明显偏绿，只有 `任务 _1233.fff` 变好。用户同时明确：两张 LR 拼接的异常是源文件问题（LR 烘焙时裁掉蓝通道），不值得为它们影响正常照片。

根因与修复（同一批中性像素上的实测，`neutral_surface_window_comparison`）：

| 样本 | v1.0.2 | 一个共享跨度 | 逐通道同像素端点 |
| --- | --- | --- | --- |
| `DSCF0895.RAF` | 0.986 · 1.022 | 1.057 · 1.599 | 1.016 · 1.068 |
| `_DSC7333.NEF` | 0.963 · 0.944 | 0.683 · 0.943 | 0.949 · 1.004 |
| `5.3-1.tif` | 0.933 · 0.964 | 0.891 · 0.673 | 0.960 · 0.991 |
| `000000070001.jpg` | 1.026 · 0.991 | 0.692 · 1.097 | 0.959 · 1.027 |
| `000021940005.jpg` | 1.017 · 1.014 | 0.685 · 1.309 | 0.990 · 1.151 |
| `精益黑白 raw0002.dng` | 1.002 · 1.018 | 0.934 · 1.164 | 1.006 · 1.026 |
| `任务 _0866.fff` | 0.964 · 0.975 | 0.796 · 1.034 | 1.034 · 1.020 |

改动：① 下线逐通道跨度响应（及其证据门、报告字段、测试）；② 有可信片基时窗口恢复为**逐通道同像素端点**（v1.0.2 的形状），不做密度域平移；③ 修正最小跨度守卫——旧守卫会在任一通道低于 0.8 D 时把三个通道全部压成 0.8，等于抹平逐通道窗口，现在只对塌缩通道按其自身中心加宽，下限 0.05 D。

整卷一致性（`G:\DCIM\755ND810` 实拍，`three_pipeline_parity_report`）：`_DSC7654 / 7655 / 7656 / 7659 / 7660` R/G = 0.992 / 1.028 / 0.974 / 0.971 / 0.941、B/G = 0.887 / 0.893 / 0.973 / 1.042 / 1.047（共享跨度下同卷 R/G 为 0.772 / 0.807 / 0.740）。

两点代价：① 片边/齿孔不再逐像素中性（实测 spread ≈ 0.02，v1.0.2 行为相同）；② 整幅色偏会被窗口吸收（乘性色偏 = 密度常量平移，片基与端点同时移动），要恢复对整幅色偏的响应只能靠物理锚点。诊断：`neutral_surface_window_comparison`、`visual_acceptance_v1_0_2_vs_v1_1`（均 `--ignored`）。
